...
3. They would need a system which could suggest tables as well as be able to flexible in managing the number of customers and the location of their table/tables.
   - The system should also be able to suggest and allocate seats at different tables if there are too many guests in one group.
...
6. They would require the ability to cancel the booking and clear the table for booking if the customer is late more than a period.
...
7. Some restaurants may require extra functionality such:
   ...
   - The ability to see the trend and success rate of each dish, maybe also allow the customers to rate their experience and give feedback.
   - The ability to track trend of sales according to calendar dates.
   - The ability to start preparing for the dish requested by the customers a period before He/She arrives so that the time taken to serve the meal could be reduced.
8. the restaurant may also have a problem with trolls or people who make a booking and do not show up. This could be an issue if the customer books many tables and did not show up as these tables wouldn't be able to be booked by other customers.

## Problem Setting
To make the program we are going to have to make some assumptions on certain features of the responses and behavior of the customer, staff and the general restaurant setting.

1. When a booking of a group is being done only one booking by a single member of the group would be placed. Multiple booking of a members of a single group will be treated as bookings of individual groups.
2. The customer will include his/her identification in the booking
3. The customer will include the timing he/she plans to arrive.
4. The Deli restaurant manager/staff will have already inputed the number of table, and the number of seat for each table, before the first booking.
5. The Deli Restaurant possess an appropiate system and OS to run the executable program.
6. If the customer makes use of any extra feature, e.g. the "prepared meal", they would fill the necessary information as prompted.
7. If any notifications are sent by the program then the staff will respond to it.
8. Staff (waiters) manually inputs the dishes ordered into the program. Along with table ID and if the bills are split then with customer ID as well.

## Functionality - How We Plan to Solve the Problems.
...
5. The Program would be able to take note of expected arrival time of customer/s and may clear the table for booking if the customer is late beyond a certain period.
6. The Program would be able to take note of trends of sales and make take note the behavior of regulars allowing for better service.
